...
export default function Navbar() {
  ...
  const [currentLang, setCurrentLang] = useState('العربية'); // Langue par défaut
        ...
        <div className="flex items-center justify-between h-24">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="relative">
              ...
              {/* Le menu déroulant */}
              {isLangOpen && (
                <div className="absolute start-0 mt-4 w-32 bg-white rounded-lg shadow-xl border border-gray-100 py-2">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => {
                        ...
                        setIsLangOpen(false);
                        // Plus tard, tu pourras appeler i18next ici
                      }}
            ...
            <Link to="/#donate" className="font-kufi px-8 py-2.5 rounded-sm text-white font-bold transition-all duration-300 shadow-md hover:shadow-lg" style={{ backgroundColor: 'var(--color-accent)' }}>
              تبرع الآن
            </Link>
          </div>
          ...
          <div className="md:hidden flex items-center gap-4">
             {/* Icône globe sur mobile aussi, sans le texte pour gagner de la place */}
        ...
        {isOpen && (
          <div className="md:hidden pb-6">
            <div className="flex flex-col space-y-4 px-2 pt-4 border-t border-gray-100">
              {navLinks.map((link, index) => (
                <Link key={index} to={link.href} className="font-kufi block text-gray-700 font-bold text-lg hover:bg-gray-50 p-2 rounded-md">
                  {link.name}
                </Link>
              ))}
              <Link to="/#donate" className="font-kufi block text-center px-4 py-3 rounded-lg text-white font-bold shadow-md mt-4" style={{ backgroundColor: 'var(--color-accent)' }}>
                تبرع الآن
              </Link>
            </div>
          </div>
